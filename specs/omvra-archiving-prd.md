...
Automatic task archival now supports opt-in completion-based delay (365 days initially) or archive on completion; the timer begins when a task enters the Done workflow category. Checks run on changes, startup and every minute while the app runs. Unknown completion dates are skipped and manual unarchive suppresses the policy until reopening and completing again. The wider milestone still requires a complete historical reporting contract and archive-heavy runtime/release acceptance. See `specs/omvra-archiving-review-2026-09-21.md` for evidence and remaining work.
