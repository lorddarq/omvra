...
export function GoalsRuntimeStatus({ element, connected, runtimeProjection, onResetExecution }: { element: GoalElement; connected: boolean; runtimeProjection?: GoalRuntimeProjection | null; onResetExecution?: () => void }) {
  ...
  return <GoalsRuntimeStatusSection>
    {executionState && <div className="mb-4 border-b border-slate-100 pb-4"><div className="flex items-center gap-1"><span>Execution</span><span className={`inline-flex w-fit items-center rounded-full border px-2.5 py-1 text-xs font-semibold capitalize ${compactChipClass(runtimeChipClass(executionState))}`}>{executionState === 'working' ? <LoaderCircle className="mr-1 size-3.5 animate-spin" /> : <CircleDot className="mr-1 size-3.5" />}{runtimeLabel(executionState)}</span></div><span className="mt-1 block text-[11px] text-slate-400">{runtimeDescription(executionState)}</span>{workerDelegationDescription(runtimeProjection?.execution?.workerDelegationStatus) && <span className="mt-2 block rounded-md border border-amber-100 bg-amber-50 px-2.5 py-2 text-[11px] text-amber-800">{workerDelegationDescription(runtimeProjection.execution.workerDelegationStatus)}</span>}{onResetExecution && <button type="button" onClick={onResetExecution} className="mt-3 rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-semibold text-slate-600 hover:border-slate-300 hover:bg-slate-50">Reset execution</button>}</div>}
